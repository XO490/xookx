...
#	Windows
	GOOS=windows GOARCH=amd64 CC="x86_64-w64-mingw32-gcc" CGO_CFLAGS="-g -O2 -w" CGO_ENABLED=1 go build -ldflags "-s -H windows" -o ./.bin/$(filename)__windows-x86_64 cmd/main.go
	#GOOS=windows GOARCH=386 CC="i386-mingw32-gcc" CGO_CFLAGS="-g -O2 -w" CGO_ENABLED=1 go build -ldflags "-s -H windows" -o ./.bin/$(filename)__windows-x86 cmd/main.go

#	Linux ARM
	#GOOS=linux GOARCH=arm64 CC="aarch64-linux-gnu-gcc" CGO_CFLAGS="-g -O2 -w" CGO_ENABLED=1 go build -ldflags "-s -H linux" -o ./.bin/$(filename)__linux-arm64 cmd/main.go
